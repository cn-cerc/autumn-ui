...
let edtSearch = new sci.TEdit(boxTitle, { label: '搜索条件：' });
...
button1.addEventListener('click', () => {
    ...
    query.open(ds => {
        ...
        if (dbn == undefined) {
            dbn = new sci.TDBNavigator(grid.getOwner()).setDataSet(dataOut);
            let edtCode = new sci.TDBEdit(grid.getOwner(), { 'label': '工号：' });
            edtCode.setDataSource(dataOut).setDataField('code_');
            let edtName = new sci.TDBEdit(grid.getOwner(), { 'label': '姓名：' });
