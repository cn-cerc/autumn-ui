...
import { imageColorArr } from "../mc/FrmMessage";
...
export default class CreateGroupDialog extends WebControl<CreateGroupDialogTypeProps, CreateGroupDialogTypeState> {
    ...
    async init() {
        ...
        let contactData = await DitengApi.searchBook(row);
        contactData.first();
        contactData.records.forEach((row: DataRow) => {
    ...
    async handleSubmit() {
        if (this.state.groupName && this.state.userCodeArr.length) {
            ...
            let ds = await DitengApi.createGroup(row);
            if(ds.state <= 0) {
                showMsg(ds.message)
            } else {
