...
export default class NewProductDialog extends BaseDialog<propsType, stateType> {
    ...
    async init() {
        let brandDataOut = await DitengApi.getBrandList();
        ...
        let class1DataOut = await DitengApi.getClass1();
        let class1List = new Map();
        class1List.set('所有大类', '');
        class1DataOut.first();
    ...
    async class1Change() {
        this.setLoad(true);
        let class2DataOut = await DitengApi.getClass2({
    ...
    async class2Change() {
        this.setLoad(true);
        let class3DataOut = await DitengApi.getClass3({
            Brand_: this.state.dataIn.getString('Brand_'),
            Class1_: this.state.dataIn.getString('Class1_'),
    ...
    initOpera(dataRow: DataRow) {
        if (this.isPhone) {
            if (dataRow.getString('Classify_') == '1') {
                ...
                return <span role='auiOpera' onClick={this.handleClick.bind(this, dataRow)}>选择</span>
        }
        else {
            if (dataRow.getString('Classify_') == '1')
                return (
    ...
    async getNewProducts(row: DataRow): Promise<DataSet> {
        this.setLoad(true);
        let NewProducts = await DitengApi.getProducts(this.state.dataIn);
        this.setLoad(false);
        return NewProducts;
    }
}