import { BaseDialog, BaseDialogStateType, DataRow, DataSet, DBEdit, SearchPanel } from "autumn-ui";
...
export default class AreaDialog extends BaseDialog<BaseAreaProps, BaseAreaState> {
    ...
    async init() {
        let areaData1 = await DitengApi.getCusArea(new DataRow())
    ...
    async search(): Promise<void> {
        ...
        try {
            ...
            let areaList = await DitengApi.getSearchCusArea(this.state.dataIn)
            if (areaList.state <= 0) {
    ...
    async handleArea1(area: string): Promise<void> {
        ...
        let areaData2 = await DitengApi.getCusArea(dataIn)
        this.setState({
            ...this.state,
            areaData2,
            areaData3: new DataSet(),
    ...
    async handleArea2(area: string): Promise<void> {
        ...
        dataIn.setValue('Area2_', this.state.area2);
        dataIn.setValue('Area3_', this.state.area3);
        let areaData3 = await DitengApi.getCusArea(dataIn)
        this.setState({
            ...this.state,
            areaData3,
        })
