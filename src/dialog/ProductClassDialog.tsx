import { BaseDialog, BaseDialogPropsType, BaseDialogStateType, DataRow, DataSet, OnSelectDataRowEvent } from "autumn-ui";
...
export default class ProductClassDialog extends BaseDialog<ProductClassTypeProps, ProductClassTypeState> {
    ...
    async init() {
        ...
        try {
            let class1 = await DitengApi.getClass1();
            ...
            if (value1)
                class2 = await DitengApi.getClass2({ 'Brand_': this.props.brand, 'Class1_': this.replaceChar(value1) });

            if (value2)
                class3 = await DitengApi.getClass3({ 'Brand_': this.props.brand, 'Class1_': this.replaceChar(value1), 'Class2_': this.replaceChar(value2) });
            this.setState({
                ...this.state,
                class1,
                class2,
    ...
    async handleClass1(name: string): Promise<void> {
        ...
        this.state.value2 = '';
        this.state.value3 = '';
        class2 = await DitengApi.getClass2({
    ...
    async handleClass2(name: string): Promise<void> {
        let class3 = new DataSet();
        this.state.value3 = '';
        class3 = await DitengApi.getClass3({
            Brand_: this.props.brand,
