/** 用户商品搜索开窗时子项商品列表开窗 */
import { BaseDialog, BaseDialogPropsType, BaseDialogStateType, Block, Column, ColumnIt, DataRow, DataSet, DBGrid, Line } from "autumn-ui";
...
export default class GetMarque extends BaseDialog<GetMarqueTypeProps, GetMarqueTypeState>{
    ...
    async init() {
        this.showAsChild();
        this.setLoad(true);
        let dataSet = await DitengApi.getSubItem({ Marque_: this.props.partCode });
        if (dataSet.state <= 0) {
