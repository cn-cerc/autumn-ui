import { BaseDialog, BaseDialogPropsType, BaseDialogStateType, Column, ColumnIt, DataRow, DataSet, DBDrop, DBEdit, DBGrid, SearchPanel } from "autumn-ui";
...
export default class ProcStepDialog extends BaseDialog<ProcStepTypeProps, ProcStepTypeState> {
    ...
    async init() {
        let dataSet1 = await DitengApi.getProcessList();
        let options = new Map();
        options.set('所有制程', '');
    ...
    async getProcSteps(): Promise<DataSet> {
        ...
        let dataSet = await DitengApi.getProcSteps(this.state.dataIn);
        this.setLoad(false);
        return dataSet;
    }
