/** 用户商品搜索开窗时子项商品列表开窗 */
import { BaseDialog, BaseDialogPropsType, BaseDialogStateType, Block, Column, ColumnIt, DataRow, DataSet, DBCheckbox, DBEdit, DBGrid, FieldMeta, Line, SearchPanel } from "autumn-ui";
...
export default class MarqueDialog extends BaseDialog<MarqueDialogTypeProps, MarqueDialogTypeState>{
    ...
    async init() {
        ...
        let dataSet = await DitengApi.getMarqueList({ Marque_: this.props.partCode, param: this.props.params, searchText });
        if (dataSet.state <= 0) {
            showMsg(dataSet.message)
        }
        let subItem = await DitengApi.getSubItem({ Marque_: this.props.partCode });
        if (subItem.state <= 0) {
        ...
        let userInfo: DataSet = await DitengApi.getUserInfo();
        if (userInfo.state <= 0) {
