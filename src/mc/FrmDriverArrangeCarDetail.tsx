...
export default class FrmDriverArrangeCarDetail extends React.Component<FrmDriverArrangeCarTypeProps, FrmDriverArrangeCarTypeState> {
    ...
    async init() {
        ...
        // 查询物流运单信息
        let driverData = await FplApi.getDriverArrangeCarDetail(driverRow);
        driverData.first();
        orderData.copyValues(driverData.current);
        // 查询货单信息
        let cargoData = await FplApi.getCargoOrderDetail(driverRow);
        cargoData.first();
        // 组装信息
        ...
        attachmentList = await FplApi.queryAttachmentList(attachmentRow);
        let btnFlag = false;
    ...
    async submitForm() {
        ...
        let dataOut = await FplApi.DriverArrangeCarModify(this.state.dataRow);
        if (dataOut['_state'] == 0) {
    ...
    async updateCodeMeter() {
        let dataOut = await FplApi.updateCodeMeter(this.state.dataRow);
        if (dataOut['_state'] != 1) {
            showMsg(dataOut['_message']);
        }
    }
