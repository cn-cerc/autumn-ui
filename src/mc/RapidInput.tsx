...
import UITree, { UIDataType, UITreeIcons, UITreeType } from "../module/UITree";
...
export default class RapidInput extends BaseDialog<RapidInputTypeProps, RapidInputTypeStates> {
    ...
    getMenus() {
        if (this.state.showMenus)
            return <UITree data={this.state.treeData} showChild={this.getTreeData.bind(this)} treeIcons={this.state.treeIcons} selectClass={styles.treeSelect} handleClick={this.setClass.bind(this)} handleDbClick={this.treeDbClick.bind(this)}></UITree>
        else
    ...
    async getBrand() {
        let dataSet = await DitengApi.getBrandList();
        dataSet.first();
        while (dataSet.fetch()) {
    ...
    async handleSearch() {
        this.setLoad(true);
        let dataSet = await DitengApi.getGatherProducts(this.state.dataIn);
        if (dataSet.state <= 0) {
    ...
    async initEnterData(row: DataRow, isFree: boolean, recNo?: number) {
        ...
        let dataOut = await DitengApi.getPartStock(dataIn);
        dataOut.first();
    ...
    async getTreeData(data: UIDataType, dataArr: any[]) {
        let dataOut: DataSet;
        let keyArr = ['Name_', 'Class2_', 'Class3_'];
        if ((!data.children || !data.children.length)) {
            if (dataArr.length == 1)
                dataOut = await DitengApi.getClass1();
            else if (dataArr.length == 2) {
                dataOut = await DitengApi.getClass2({
                    Brand_: '',
                    Class1_: dataArr[1],
                });
            } else if (dataArr.length == 3) {
                dataOut = await DitengApi.getClass3({
                    Brand_: '',
                    Class1_: dataArr[1],
                    Class2_: dataArr[2],
                });
    ...
    isBodyExists() {
        ...
        for (let i = 0; i < this.state.data.records.length; i++) {
            items.push(this.state.data.records[i].getString('Code_'));
        }
        ...
        fetch(`${url}?products=${encodeURIComponent(products)}&flag=true`).then(response => response.json()).then((data: any) => {
            if (data.result) {
                this.handleSubmit(1);
            } else {
                this.setState({
