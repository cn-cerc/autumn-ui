...
export default class FrmDriverReceive extends WebControl<
    FrmDriverReceiveTypeProps,
    FrmDriverReceiveTypeState
> {
            ...
            return (
                <React.Fragment>
                    ...
                    <ul className={styles.orderTypeList}>
                        <li
                            className={this.state.orderType == 1 ? styles.orderActive : ""}
                            onClick={() => this.setState({ orderType: 1 })}
                        >
                            未完成
                        </li>
                        <li
                            className={this.state.orderType == 2 ? styles.orderActive : ""}
                            onClick={() => this.setState({ orderType: 2 })}
                        >
                            已完成
                        </li>
                    </ul>
    ...
    async init() {
        ...
        while (orderData.fetch()) {
            if (
                orderData.getString("confirm_status_") == "0" ||
                orderData.getString("delivery_status_") < "4"
            ) {
            ...
            if (orderData.getString("confirm_status_") == "0") {
                ...
                if (!this.isPhone) {
                    ...
                    // 目的地
                    let receiveSite2 = gridData
                        .getString("destination_")
                        .replace(/\//g, "");
                    let sendSite =
                        sendSite1.indexOf(sendSite2) > -1
                            ? sendSite1
                            : sendSite2 + sendSite1;
                    let receiveSite =
                        receiveSite1.indexOf(receiveSite2) > -1
                            ? receiveSite1
                            : receiveSite2 + receiveSite1;
            ...
            } else gridData_.append().copyRecord(orderData.current);
        }
        if (gridData_.size > 1) gridData_.setSort("receiving_time_ ASC");
        this.setState({
            notData: gridData,
        ...
        if (this.state.isInit) {
            let jsx = (
                <div className={styles.noGrid}>
                    暂无未接单的物流订单，前往<a href="FrmCarGrab">抢单大厅</a>
                </div>
            );
                ...
                jsx = (
                    <React.Fragment>
                        <div className={styles.gridTitle}>
                            您存在未接物流运单——{this.state.notData.size}单，请尽快接单
                        </div>
                        <DBGrid dataSet={this.state.notData} className={styles.dbgrid}>
                            <Column
                                code="send_date_time_"
                                width="16"
                                name="发货时间"
                            ></Column>
                            <Column
                                code="arrive_date_time_"
                                width="16"
                                name="到货时间"
                            ></Column>
                            ...
                            <Column
                                ...
                                width="6"
                                name="操作"
                                customText={(row: DataRow) => {
                                    return (
                                        <span
                                            className={styles.opera}
                                            onClick={this.handleSelect.bind(this, row)}
                                        >
                                            接单
                                        </span>
                                    );
                            ...
                            {this.state.showWay ? (
                                <Column
                                    ...
                                    name=""
                                    customText={(row: DataRow) => {
                                        return row.getBoolean("hasWay") ? (
                                            <span
                                                className={styles.opera}
                                                onClick={this.toGaode.bind(this, row)}
                                            >
                                                查看路线
                                            </span>
                                        ) : (
    ...
    getOrderList() {
        ...
        let list = [];
        let ds = new DataSet();
        if (this.state.orderType == 1) ds.appendDataSet(this.state.notComplete);
        else ds.appendDataSet(this.state.isCompleted);
        ds.first();
        ...
        while (ds.fetch()) {
            let isReceived = true;
            if (ds.getString("confirm_status_") == "0") isReceived = false;
            list.push(
                this.getOrderDetail(hasNextOrder, ds.current, isReceived, time, index)
            );
            index++;
        }
        ...
        if (bool) {
            list.push(
                <li className={styles.noOrder} key="noData">
                    <div>
                        <img
                            src={StaticFile.getImage("images/Frmshopping/notDataImg.png")}
                            alt=""
                        />
                    </div>
                    <p>暂无运单</p>
        ...
        return (
            <ul
                ...
                style={{ marginTop: !bool ? "0.75rem" : "" }}
                key={this.state.orderType}
            >
                {list}
            </ul>
        );
    ...
    getToast() {
        ...
        let jsx = (
            <p>
                暂无接单，快去抢单大厅看看吧~！
                <a style={{ marginLeft: "12px" }} href="FrmCarGrab">
                    立即查看
                </a>
            </p>
        );
        if (this.state.receivedData.size > 0) {
            ...
            for (let i = 0; i < this.state.receivedData.records.length; i++) {
                let time_ = new Date(
                    this.state.receivedData.records[i].getString("receiving_time_")
                ).getTime();
                let _time = new Date(
                    this.state.receivedData.records[
                        this.state.receivedData.records.length - i - 1
                    ].getString("receiving_time_")
                ).getTime();
                if (time_ > time && !nearTime) {
                    nearTime = time_;
                    nearHref = `FrmDriverArrangeCar.detail?cargoNo=${this.state.receivedData.records[
                        i
                    ].getString("cargo_no_")}&tbNo=${this.state.receivedData.records[
                        i
                    ].getString("tb_no_")}&dcorpno=${this.state.receivedData.records[
                        i
                    ].getString("d_corp_no_")}&it=${this.state.receivedData.records[
                        i
                    ].getDouble("it_")}`;
                }
                if (_time <= time && !lastTime) lastTime = _time;
                if (nearTime && lastTime) break;
            }
            if (nearTime) {
                jsx = (
                    <p style={{ color: "#E65C5C" }}>
                        您距离下一单发货时间还有{Math.ceil((nearTime - time) / 3600000)}
                        个小时！
                        <a style={{ marginLeft: "12px" }} href={nearHref}>
                            立即查看
                        </a>
                    </p>
                );
                ...
                jsx = (
                    <p>
                        您已经{Math.floor((time - lastTime) / 86400000)}
                        天没有接单了，快来抢单大厅看看吧！
                        <a style={{ marginLeft: "12px" }} href="FrmCarGrab">
                            立即查看
                        </a>
    ...
    getOrderDetail(
        ...
        if (!hasNextOrder && isReceived && !hasNextOrder && time_ > time) {
            ...
            return (
                <li
                    key={this.state.notData.recNo}
                    onClick={this.handleSelect.bind(this, row)}
                >
                    <div className={styles.orderMsg}>
                        <img src="images/icon/error.png" />
                        <span>
                            您距离下一单发货时间还有{Math.ceil((time_ - time) / 3600000)}
                            个小时
                        </span>
                    </div>
                    <div className={styles.orderDetail}>
                        ...
                        <button onClick={this.handleSelect.bind(this, row)}>
                            查看详情
                        </button>
                    </div>
                </li>
                ...
                >
                    <div className={styles.orderTop}>
                        <div>
                            ...
                            {this.state.orderType == 1 ? (
                                <span
                                    onClick={(e) => {
                                        e.preventDefault(), e.stopPropagation(), this.toGaode(row);
                                    }}
                                >
                                    {" "}
                                    <a className={styles.luxian} href="">
                                        {" "}
                                        <img
                                            src={StaticFile.getImage("images/Frmshopping/site.png")}
                                            alt=""
                                        />{" "}
                                        路线
                                    </a>{" "}
                    ...
                    <div className={styles.siteCenter}>
                        <div className={styles.siteInfo}>
                            <span className={styles.siteSkin}>{`${row
                                .getString("depart_")
                                .replace(/\\/g, "")}${row.getString("send_detail_")}`}</span>
                            <span className={styles.siteSkin}>{`${row
                                .getString("destination_")
                                .replace(/\\/g, "")}${row.getString("receive_detail_")}`}</span>
                        </div>
                    </div>
                    <div className={styles.orderCenter}>
                        <div className={styles.orderInfo}>
                            <span>
                                {row.getString("code_")} |{" "}
                                {this.fromatPriceFun(row.getString("num_"))}
                                {[this.unitArr[row.getDouble("main_unit_")]]}{" "}
                                {row.getInt("driver_type_") == 1
                                    ? `| ${this.fromatPriceFun(
                                        row.getString("unit_price_")
                                    )}元/${[this.unitArr[row.getDouble("main_unit_")]]}`
                                    : ""}
                            </span>
                    ...
                    <div className={styles.orderBottom}>
                        <div className={styles.freight}>
                            ...
                            <span>
                                {row.getInt("driver_type_") == 1
                                    ? math.toFixed(row.getString("amount_"), 2)
                                    : ""}
                            </span>
                        </div>
                        <div className={styles.btns}>
                            {isReceived ? (
                                <button className={styles.btn_detail}>详情</button>
                            ) : (
                                <button className={index == 1 ? "" : styles.defaultBtn}>
                                    接单
                                </button>
                            )}
                        </div>
    ...
    handleSelect(row: DataRow) {
        location.href = `FrmDriverArrangeCar.detail?cargoNo=${row.getString(
            "cargo_no_"
        )}&tbNo=${row.getString("tb_no_")}&dcorpno=${row.getString(
            "d_corp_no_"
        )}&it=${row.getDouble("it_")}`;
    }
    ...
    async toGaode(row: DataRow) {
        if (this.isPhone) {
            ...
            let sendSite =
                sendSite1.indexOf(sendSite2) > -1 ? sendSite1 : sendSite2 + sendSite1;
            let receiveSite =
                receiveSite1.indexOf(receiveSite2) > -1
                    ? receiveSite1
                    : receiveSite2 + receiveSite1;
            ...
            if (
                row.getBoolean("receiveGeocoder") &&
                row.getBoolean("receiveGeocoder")
            ) {
                let sendGeocoder = row.getValue("sendGeocoder");
                let receiveGeocoder = row.getValue("receiveGeocoder");
                window.open(
                    `https://uri.amap.com/navigation?from=${sendGeocoder.site.join()},${sendGeocoder.address
                    }&to=${receiveGeocoder.site.join()},${receiveGeocoder.address
    ...
    formatDateTimeFun(dateObj: Date) {
        ...
        if (dateObj.getFullYear == new Date().getFullYear) {
            str = `${dateObj.getMonth() + 1 < 10
                ? "0" + (dateObj.getMonth() + 1)
                : dateObj.getMonth() + 1
                }月${dateObj.getDate() < 10 ? "0" + dateObj.getDate() : dateObj.getDate()
                }日${dateObj.getHours() < 10 ? "0" + dateObj.getHours() : dateObj.getHours()
                }时`;
        } else {
            str = `${dateObj.getFullYear()}年${dateObj.getMonth() + 1 < 10
                ? "0" + (dateObj.getMonth() + 1)
                : dateObj.getMonth() + 1
                }月${dateObj.getDate() < 10 ? "0" + dateObj.getDate() : dateObj.getDate()
                }日`;
