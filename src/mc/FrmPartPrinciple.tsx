import { Column, ColumnIt, DataRow, DataSet, DBEdit, DBGrid, MainRow, SearchPanel } from "autumn-ui";
...
export default class FrmPartPrinciple extends React.Component<FrmPartPrincipleTypeProps, FrmPartPrincipleTypeState> {
    ...
    async init() {
        let dataSet1 = await DitengApi.getCodeClass();
        this.setState({
            dataSet1
        })
    }
    ...
    async selectClass(row: DataRow) {
        ...
        let dataSet2 = await DitengApi.getPartPrincipleSearch(dataRow2);
        this.setState({
            titleList: [{
                text: '大类'
            }, {
    ...
    async selectClass2(row: DataRow) {
        ...
        let codeSchemeData = await DitengApi.getPartPrincipleDownload(dataRow);
            ...
            while (codeSchemeData.fetch()) {
                if (codeSchemeData.getDouble('Type_') == 0) {
                    ...
                    if (!isFirst) {
                        ...
                        specCode = specCode_;
                        dataSet3 = await DitengApi.getPartSpecDownload(dataIn);
    ...
    async choseClass3(key: number, specCodeData?: DataSet) {
        ...
        let dataSet3 = await DitengApi.getPartSpecDownload(dataIn);
        let _dataSet3 = new DataSet();
        _dataSet3.appendDataSet(dataSet3);
        let searchRow = new DataRow();
    ...
    async initButton() {
        ...
        if (isAllSelect) {
            ...
            let dataOut = await DitengApi.existsPartInfo(headIn);
            if (dataOut.state > 0) {
    ...
    async handleSubmit() {
        ...
        if (this.state.dataRow.getValue('PartCode_').length != ds.head.getDouble('CodeLen_')) {
            ...
            let dataOut = await DitengApi.postPartStock(dataIn);
            if (dataOut.state < 1) {
                ...
                let ds2 = await DitengApi.updatePartPrinciple(dataRow);
                if (ds2.state < 1) {
                    ...
                    DitengApi.createSubitemBOM(dataSet);
                    this.addDetail();
                }
            }
            this.setState({
    ...
    async addDesc() {
        try {
            ...
            let ds = await DitengApi.getPartSpecDownload(dataIn);
            if (ds.state < 1) {
                showMsg(ds.message);
                return;
            }
            ...
            let dataOut = await DitengApi.getPartSpecModify(ds2);
            if (dataOut.state > 0) {
                showMsg('添加成功!');
