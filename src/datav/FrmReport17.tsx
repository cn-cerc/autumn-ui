...
export default class FrmReport17 extends React.Component<FrmReportTypeProps, FrmReportTypeState> {
    ...
    async init() {
        ...
        reportHead
            ...
            .setValue('A3', { name: '未出货数量（吨）', width: '8' })
            .setValue('A4', { name: '当前库存（吨）', width: '6' })
            .setValue('A5', { name: '金额（元）', width: '4' })
            .setValue('A6', { name: '销售目标（吨）', width: '5' })
    ...
    render(): React.ReactNode {
        return <ReportDetail dataSet={this.state.reportData} head={this.state.reportHead} title='线材未出货订单与库存动态（2022年）' key={this.state.reportData.json}></ReportDetail>
    }
}