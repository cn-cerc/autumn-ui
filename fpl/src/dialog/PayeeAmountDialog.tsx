...
import { DataRow, DataSet, BaseDialogStateType, BaseDialog, BaseDialogPropsType, SearchPanel, DBEdit, DBGrid, Column, ColumnIt, OperatePanel } from "autumn-ui";
import FplDialogApi from "./FplDialogApi";
import styles from "./DialogCommon.css";
import "../tool/Summer.css";
import { AuiMath, showMsg } from "@diteng/tool/Summer";
...
export default class PayeeAmountDialog extends BaseDialog<PayeeProps, StaffTypeState> {
    ...
    async init() {
        this.setLoad(true);
        let dataSet = await FplDialogApi.getPayeeCode(this.state.dataIn);
        this.setLoad(false);
        this.setState({
            dataSet
        })
