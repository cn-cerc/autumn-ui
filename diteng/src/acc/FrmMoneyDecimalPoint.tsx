...
import { SClient, DataRow, MenuItem, ToolPanel, ToolItem, SearchPanel, DBEdit, DBGrid, Column, OperatePanel, DataSet } from "autumn-ui";
...
export default class FrmMoneyDecimalPoint extends CustomForm<CustomFormPropsType, MoneyRateTypeState> {
    ...
    renewUpdateDate() {
        this.state.client.first();
        let updateTime = new Datetime().toString();
        while(this.state.client.fetch()) {
            let itemMap = this.state.client.current.items;
            let histroy = this.state.client.current.history;
            if(histroy && !Utils.equalsMap(itemMap, histroy.items)) {
                this.state.client.setValue('UpdateDate_', updateTime)
            }
        }
    }
}