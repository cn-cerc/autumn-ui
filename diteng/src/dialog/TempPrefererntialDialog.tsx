...
export default class TempPrefererntialDialog extends BaseDialog<BaseDialogPropsType, TempPrefererntialTypeState> {
    ...
    async btnExecute(sender: any) {
        ...
        let dataOut = await DialogApi.appendTempPrefererntial(this.state.dataIn);
        if (dataOut.state <= 0) {
    ...
    async handleRemove(row: DataRow) {
        let dataOut = await DialogApi.removeTempPrefererntial({ UID_: row.getString('UID_') });
        if (dataOut.state <= 0) {
            showMsg(dataOut.message)
        } else {
            let dataSet = await this.getTempPrefererntial();
    ...
    async getTempPrefererntial() {
        this.setLoad(true);
        let dataSet = await DialogApi.getTempPrefererntial();
        this.setLoad(false);
        return dataSet;
    }
}