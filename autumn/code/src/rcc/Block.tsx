...
export default class Block extends Control<propsType, stateType> {
    ...
    render() {
        return (
            <div className='aui-block-block'>
                {this.getRows()}
                {this.state.rowMax <= this.props.dataSet.size ? <div id='more' className='aui-block-more'>总记录数：{this.props.dataSet.size}</div> : ''}
            </div>
        )
    }
    ...
    getRows(): React.ReactNode[] {
        ...
        while (ds.fetch()) {
            ...
            items.push(
                <div className='aui-block-row' key={`master_${recNo}`} data-key={`master_${recNo}`} role='tr' onClick={this.onTrClick}>
            ...
            React.Children.map(this.props.children, child => {
                if (isValidElement(child) && child.type == ChildRow) {
                    ...
                    if (child.props.visible || (child.props.autoJudge && isHide))
                        display = 'none'
                    items.push(
                        <div className='aui-block-row aui-block-childRow' key={`child_${key}`} id={`child_${key}`} data-key={`child_${key}`} role='tr' onClick={this.onTrClick}>
...
export class Line extends Control<LinePropsType, LineTypeState> {
    ...
    render() {
        return (
            <div className={this.props.className ? 'aui-block-line' + ` ${this.props.className}` : 'aui-block-line'}>
                {this.getRow()}
            </div>
        )
    }
