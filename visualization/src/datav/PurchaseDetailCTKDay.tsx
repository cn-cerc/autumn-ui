...
export default class PurchaseDetailCTKDay extends React.Component<FrmReportTypeProps, FrmReportTypeState> {
    ...
    async init() {
        ...
        await fetch('./铁矿石.xls', {
            ...
            while(ks.fetch()) {
                ...
                if(ks.getString('种类') == '磁铁矿' &&  month == m && day == d ) {
                    reportData.append().
    ...
    render(): React.ReactNode {
        return <ReportDetail params={{index:1}} dataSet={this.state.reportData} head={this.state.reportHead} title='磁铁矿今日入库数量（T）' key={this.state.reportData.json} backHref={FrmPurchaseChart3} backTitle='工业4.0-数字化供应链管理中心V1.0'></ReportDetail>
    }
}