...
export default class FrmReport14 extends React.Component<FrmReportTypeProps, FrmReportTypeState> {
    ...
    render(): React.ReactNode {
        return <ReportDetail dataSet={this.state.reportData} head={this.state.reportHead} title={`${this.state.reportName}本周出货动态（${new Date().getMonth() + 1}月${new Date().getDate()}日）`} key={this.state.reportData.json} backHref='FrmSaleChart' backTitle='销售数据管理中心'></ReportDetail>
    }
}